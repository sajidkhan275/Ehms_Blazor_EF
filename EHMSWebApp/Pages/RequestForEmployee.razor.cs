...
namespace EHMSWebApp.Pages
{
    public partial class RequestForEmployee
    {
        ...
        private async Task LoadDetails()
        {
            ...
            {
                requestForHelp = (await requestForHelpService.GetRequestsByEmployee()).ToList();
            }
            else
            {
                requestForHelp = (await requestForHelpService.GetRequestsByEmployeeId(EmpId)).ToList();
            }
            FilterData();
            PaginateData();
        }
        ...
        private async Task SavePhysicalFitness()
        {
            if (isEdit)
            {
                ...
                await requestForHelpService.UpdateHRRequest(currentrequestForHelp);
            }
            else
            {
                currentrequestForHelp.Status = "Pending";
                currentrequestForHelp.EmpId = EmpId;
                await requestForHelpService.CreateRequestForHelp(currentrequestForHelp);
            }
        ...
        {
            await requestForHelpService.DeleteRequestForHelp(id);
            await LoadDetails();
        }
