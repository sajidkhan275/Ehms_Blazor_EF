...
namespace EHMSWebApp.Pages
{
    public partial class EmployeeManagement
    {
        ...
        private async Task LoadEmployees()
        {
            ...
            EmployeeWithRoleDetails employeeWithRoleDetails = (await employeeService.GetRoleEmpWise(entraId));

            if (employeeWithRoleDetails != null)
            {
                UserRoles = employeeWithRoleDetails?.EmployeeRoles?.Select(x => x.RoleId).ToList();
            }
            if(UserRoles == null) { UserRoles = new List<int>(); }
           

            employees = (await employeeService.GetAllEmployees()).OrderBy(x => x.EmployeeCode).ToList();
