...
namespace EHMSWebApp.Pages
{
    public partial class PhysicalFitnessManagement
    {
        ...
        private async Task LoadDetails()
        {
            ...
            {
                physicalFitness = (await physicalFitnessService.GetEmployeeHealthInfoAsync()).ToList();
            }
            else
            {
        ...
        private async Task SavePhysicalFitness()
        {
            ...
            if (isEdit)
            {
                await physicalFitnessService.UpdateEmployeePhysicalFitness(currentphysicalFitness);
            }
            else
            {
                await physicalFitnessService.CreateEmployeePhysicalFitness(currentphysicalFitness);
            }
        ...
        {
            await physicalFitnessService.DeleteEmployeePhysicalFitness(id);
            showDeleteDialog = false;
            deleteRecordId = 0;
            await LoadDetails();
        }
